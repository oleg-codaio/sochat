...
public class ChatClient implements Runnable {
	 ...
	 */
	public ChatClient(InetAddress serverAddress, int serverPort, UserIO logger)
			throws IOException {
		mUserIO = logger;
		...
		public void run() {
			while (true) {
				...
				try {
					...
					mUserIO.logError("Error receiving packet " + packet + ": "
							+ e);
					// e.printStackTrace();
					continue;
				}
				...
				if (messageType < 0
						|| messageType >= MessageType.values().length) {
				...
				case INCOMING:
					...
					String msg = new String(packet.getData(), contentOffset,
							contentLen);
	...
	public static void main(String args[]) {
		...
		ChatClient server;
		try {
			server = new ChatClient(serverAddress, serverPort,
					new StandardUserIO());
			server.run();
	...
	private static void printUsage() {
		System.out
				.println("SOChat, by Oleg and Saba for CS4740 final project\n\n"
